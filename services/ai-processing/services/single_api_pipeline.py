...
请以 JSON 格式返回，包含：
- intent：**必填**，用一句简洁的中文概括测试目标（不要直接复制用户原话，要提炼出具体的测试意图）。例如：
  - 用户说「为登录接口生成测试」→ intent 应为「验证用户手机号密码登录及各种异常场景」
  - 用户说「测试下单」→ intent 应为「验证下单接口的正常下单与异常参数处理能力」
  - 应包含：接口功能名称、测试覆盖的核心场景（正向/异常/边界）

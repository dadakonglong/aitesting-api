...
echo "=== 剩余文件 ==="
ls /etc/systemd/system/docker.service.d/ 2>/dev/null || echo "目录已空"
...
""", "彻底删除所有 Docker 代理配置", timeout=60)

    # ===== 测试 =====
    run(c, "docker pull hello-world 2>&1 | tail -5", "测试 docker pull 可用性", timeout=120)

    run(c, "docker pull python:3.11-slim 2>&1 | tail -5", "拉取 Python 3.11", timeout=300)
    run(c, "docker pull nginx:alpine 2>&1 | tail -5", "拉取 Nginx", timeout=300)
    run(c, "docker pull node:18-alpine 2>&1 | tail -5", "拉取 Node 18", timeout=300)
